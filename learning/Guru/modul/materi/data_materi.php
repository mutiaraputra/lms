                ...
                <div class="card-body">
                  ...
                  <table class="table table-condensed table-hover" id="data">
                   ...
                   <div class="modal fade bs-example-modal-lg" id="<?=$row['id_materi']; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                      ...
                      ?></td>
                      <td>
                      <button class="btn btn-info btn-sm view-readers-btn" data-toggle="modal" data-target="#readersModal<?php echo $row['id_materi']; ?>">Dibaca Oleh..</button>
                      <!-- Modal to display students who have read the material -->
                      <div class="modal fade" id="readersModal<?php echo $row['id_materi']; ?>" tabindex="-1" role="dialog" aria-labelledby="readersModalLabel" aria-hidden="true">
                      <div class="modal-dialog" role="document">
                          <div class="modal-content">
                              <div class="modal-header">
                                  <h5 class="modal-title" id="readersModalLabel">Siswa yang Membaca Materi "<?php echo $row['judul_materi']; ?>"</h5>
                              ...
                              <div class="modal-body">
                                  <ul>
                                      <?php
                                      // Query untuk mendapatkan daftar siswa yang telah membaca materi
                                      $id_materi = $row['id_materi']; // Ambil id_materi dari data baris saat ini
                                      $sql = "SELECT nama_siswa FROM tb_materibaca WHERE id_materi = $id_materi";
                                      $result = mysqli_query($con, $sql);

                                      // Cek jika ada hasil
                                      if ($result && mysqli_num_rows($result) > 0) {
                                          while ($reader = mysqli_fetch_assoc($result)) {
                                              echo "<li>" . htmlspecialchars($reader['nama_siswa']) . "</li>";
                                          }
                                      } else {
                                          echo "<p>Belum ada siswa yang membaca materi ini.</p>";
                                      }
                                      ?>
                                  </ul>
                              </div>
                      ...
                      <td>
                        <?php
                        if ($row['public']=='Y') {
                          echo "<a href='?page=materi&act=activate&id=$row[id_materi]&status=$row[public]' class='btn btn-danger btn-sm text-white'><i class='fa fa-spin fa-lock'></i></a>";
                        } else {
                          echo "<a href='?page=materi&act=activate&id=$row[id_materi]&status=$row[public]' class='btn btn-success btn-sm text-white'><i class='fa fa-spin fa-unlock'></i></a>";
                        }
                        ?>
                      </td>
                      <td>
                        <a href="?page=materi&act=edit&ID=<?=$row['id_materi'] ?>" class="btn btn-dark btn-sm"><i class="fa fa-edit"></i> </a>
                      </td>
                      <td>
                        <a href="?page=materi&act=del&ID=<?=$row['id_materi'] ?>" onclick="return confirm('Yakin Hapus Data ?')" class="btn btn-dark text-danger btn-sm"><i class="fa fa-trash"></i> </a>
                      </td>
                    </tr>
                    <?php } ?>
