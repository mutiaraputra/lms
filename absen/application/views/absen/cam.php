...
$allowed_ips = ['127.0.0.1', '192.168.1.2']; // IP yang diizinkan

// Cek apakah IP pengunjung ada dalam daftar IP yang diizinkan
if (!in_array($ip_address, $allowed_ips)) {
    // Jika IP tidak diizinkan, tampilkan pesan atau arahkan ke halaman lain
    header('HTTP/1.0 403 Forbidden');
    echo "IP ($ip_address) tidak diijinkan.";
    exit;
}
*/
...
<body>
    ...
    <div class="footer">
        <div class="copyright">
            <p>Developed By <a href="http://smknagara.id/" target="_blank">SMK Nagara</a> 2024</p>
        </div>
    </div>
    <!--**********************************
            Footer end
    ...
    ***********************************-->
    ...
    <script type="module">
        ...
        const absenberakhirmasuk = "<?= $absenberakhirmasuk ?>";
        const absenkeluar = "<?= $absenkeluar ?>";
        const absenakhirkeluar = "<?= $akhirkeluar ?>";

        // Pisahkan jam, menit, dan detik dari string waktu
        const [jam1, menit1, detik1] = absenmasuk.split(":");
        const [jam2, menit2, detik2] = absenberakhirmasuk.split(":");
        const [jam3, menit3, detik3] = absenkeluar.split(":");
        const [jam4, menit4, detik4] = absenakhirkeluar.split(":");

        let r = new Date();
        //const bulan2 = r.toLocaleString('default', { month: 'short' }) || 'Oct';  // Default to 'Jan' if not available
        const bulan2 = r.getMonth();
        const tanggal = r.getDate() || 13;  // Default to 1
        const tahun = r.getFullYear() || 2024;  // Default to 2023
        //const jammasuk = `"${bulan2} ${tanggal}, ${tahun} 07:00:00"`
        const now = new Date().getTime();
        //const jammasuk = new Date(`${bulan2} ${tanggal}, ${tahun} <?= $absenmasuk ?>`).getTime();
        //const jammasuk = new Date(`${bulan2} ${tanggal}, ${tahun} <?= $absenmasuk ? $absenmasuk : '07:00:00' ?>`).getTime();
        //const akhirmasuk = new Date(`${bulan2} ${tanggal}, ${tahun} <?= $absenberakhirmasuk ?>`).getTime();
        //const akhirmasuk = new Date(`${bulan2} ${tanggal}, ${tahun} <?= $absenberakhirmasuk ? $absenberakhirmasuk : '21:00:00' ?>`).getTime();
        const jammasuk = new Date(tahun, bulan2, tanggal, jam1, menit1, detik1).getTime();
        const jammasuk2 = new Date(tahun, bulan2, tanggal+1, 7, 0, 0).getTime();
        const akhirmasuk = new Date(tahun, bulan2, tanggal, jam2, menit2, detik2).getTime();
        const keluar = new Date(tahun, bulan2, tanggal, jam3, menit3, detik3).getTime();
        const akhirkeluar = new Date(tahun, bulan2, tanggal, jam4, menit4, detik4).getTime();
        //const keluar = new Date(`${bulan2} ${tanggal}, ${tahun} <?= $absenkeluar ?>`).getTime();
        //const keluar = new Date(`${bulan2} ${tanggal}, ${tahun} <?= $absenkeluar ? $absenkeluar : '21:01:00' ?>`).getTime();
        //const akhirkeluar = new Date(`${bulan2} ${tanggal}, ${tahun} <?= $akhirkeluar ?>`).getTime();
        //const akhirkeluar = new Date(`${bulan2} ${tanggal}, ${tahun} <?= $akhirkeluar ? $akhirkeluar : '23:59:00' ?>`).getTime();
...
/*
        scanner.start().then(() => {
            ...
            var x = setInterval(function() {
                ...
                var days = distance > 0 ? Math.floor(distance / (1000 * 60 * 60 * 24)) : 0;  // Only apply if distance > 0
                ...
                $('.informasi').html(`<h1><?= $akhirkeluar ?></h1>`)
