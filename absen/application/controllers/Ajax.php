...
class Ajax extends CI_Controller
{
    ...
    public function inputabsen()
    {
        ...
        if ($this->M_siswa->CekSiswa($nis) < 1) {
            ...
            if ($type == 'masuk') {
                ...
                $cekabsenkeluar = $this->db->query("SELECT * FROM tabel_detail_absen WHERE nis = '$nis' AND keluar = '1' AND tanggal_absen = '$tglskrg' ")->num_rows();
                if ($cekabsen == 0) {
    ...
    public function inputabsenguru()
    {
        ...
        if ($this->M_guru->CekGuru($nik) < 1) {
            ...
            $tglskrg = date('Y-m-d');
            $cekabsen = $this->db->query("SELECT * FROM tabel_detail_absen_guru WHERE nik = '$nik' AND masuk = 1 AND tanggal_absen = '$tglskrg' ")->num_rows();
            if ($type == 'masuk') {
                if ($cekabsen > 0) {
                    $result2 = ['status' => 'false', 'alert' => 'success', 'message' => 'Anda sudah absen masuk hari ini, selamat beraktivitas :)'];
                    echo json_encode($result2);
                } else {
                    $dataguru = $this->M_guru->detailguru($nik);
                    $dataguru2 = $dataguru[0];
                    $this->M_absensi->inputAbsenByGuru($dataguru2, $absen, $type);
                    $pesannya = 'Bpk/Ibu ' . $dataguru2['nama_guru'] . ' telah absen masuk hari ini pada pukul ' . date('H:i:s');
                    $this->M_notif->send_whatsapp($dataguru2['no_telepon'], $pesannya);
                    echo json_encode(array_merge($dataguru2, ['status' => 'true']));
                }
            } else if ($type == 'keluar') {
                $cekabsenkeluar = $this->db->query("SELECT * FROM tabel_detail_absen_guru WHERE nik = '$nik' AND keluar = '1' AND tanggal_absen = '$tglskrg' ")->num_rows();
                if ($cekabsen == 0) {
                    $result3 = ['status' => 'false', 'alert' => 'danger', 'message' => 'Anda tidak bisa absen keluar , karna tidak ada data absen masuk kamu hari ini!'];
                    echo json_encode($result3);
                } else if ($cekabsenkeluar > 0) {
                    $result3 = ['status' => 'false', 'alert' => 'success', 'message' => 'Anda sudah absen keluar hari ini, selamat beristirahat!'];
                    echo json_encode($result3);
                } else {
                    $this->db->query("UPDATE tabel_detail_absen_guru SET keluar = '1' WHERE nik ='$nik' AND tanggal_absen = '$tanggal' AND masuk = '1'");
                    $dataguru = $this->M_guru->detailguru($nik);
                    $dataguru2 = $dataguru[0];

                    $pesannya = 'Bpk/Ibu ' . $dataguru2['nama_guru'] . ' telah absen keluar hari ini pada pukul ' . date('H:i:s');
                    $this->M_notif->send_whatsapp($dataguru2['no_telepon'], $pesannya);
                    echo json_encode(array_merge($dataguru2, ['status' => 'true']));
                }
            }
    ...
    // student
    public function linked_nik()
    {

        $nik = $this->input->post('nik');
        $tanggal = date('Y-m-d');
        //$kodeunik = base64_encode($nik . '=' . $tanggal . '=' . random_int(10000000, 999999999));
        $kodeunik = 'k';
        $ceknik = $this->db->query("SELECT * FROM tabel_guru WHERE nik = '$nik'");
        $cekuser = $this->db->query("SELECT * FROM login_guru WHERE nik_guru = '$nik' AND is_active = '0'")->num_rows();
        if ($ceknik->num_rows() < 1 || $cekuser > 0) {
            echo json_encode('already');
        } else {
            $dataguru = $ceknik->result_array()[0];
            /*
            $message =
                '
            Hai ' . $dataguru['nama_guru']  . ' Kamu telah melakukan pengaitan akun,
            berikut link untuk meneruskan pengaitan akun anda 
            ' . base_url() . 'student/auth/linked_proccess/' . $kodeunik . '
            ...
            */
            $message =
                '
            Hai ' . $dataguru['nama_guru']  . ' Anda telah melakukan pengaitan akun';
            ...
            $this->db->insert('login_guru', $data);
            echo json_encode('success');
        }
    }
    ...
    public function login_guru()
    {
        $nik = $this->input->post('nik');
        $password = $this->input->post('password');
        $guru = $this->db->get_where('login_guru', ['nik_guru' => $nik])->result_array()[0];
        $bioguru = $this->db->get_where('tabel_guru', ['nik' => $nik])->result_array()[0];

        if ($guru && $bioguru) {
            if ($guru['is_active'] == 1) {
                if (password_verify($password, $guru['password'])) {
                    $dataguru = $bioguru;
                    $sessionguru = array_merge($dataguru, ['level' => 'guru']);
                    $this->session->set_userdata($sessionguru);
                    $result = ['status' => true, 'data' => base_url() . 'teacher/dashboard'];
                } else {
                    $result = ['status' => false, 'data' => 'Nik atau password salah!'];
                }
            } else {
                $result = ['status' => false, 'data' => 'Email belum di aktivasi'];
            }
    ...
    public function verif_email_siswa()
    {
        ...
        if ($querynis->num_rows() < 1) {
            ...
            $this->M_notif->send_data($message);
			if ($this->M_notif->send_email($email, 'Verifiksai email', $message) == true) {
                $data = ['status' => true, 'message' => $email];
            } else {
    ...
    public function save_password_siswa()
    {
        $nis = $this->session->userdata('nis');
        $pwbaru = $this->input->post('newpw');
        ...
        $data = ['status' => true, 'message' => 'Berhasil Menyimpan Kata Sandi'];
