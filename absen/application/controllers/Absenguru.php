...
class Absenguru extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->input->get('cariabsen') != FALSE) {
            ...
            if ($this->input->get('nik') != FALSE) {
                $nik = $this->input->get('nik');
                $dataguru = $this->M_absensi->CariGuru($nik);
                // deskripsi pencarian dengan nik
                $desc = 'nik ' . $nik . ' ( ' . $namabulan[$bulan] . ' ' . $tahun . ' )';
            }else{
                $dataguru = $this->M_absensi->dataGuru($nik);
                $desc = ' ( ' . $namabulan[$bulan] . ' ' . $tahun . ' )';
            }
        }
        $data = [
        ...
        $this->load->view('absenguru/index');
        ...
        if (date('D', strtotime($tanggal)) == 'Sun' && $minggu[0]['status'] == 'Aktif') {
            ...
            foreach ($this->input->post('nik') as $n) {
                ...
                if ($this->input->post('aksi') == 'baru') {
                    $cekabsen = $this->db->query("SELECT * FROM tabel_detail_absen WHERE nik = '$n' AND tanggal_absen = '$tanggal' ");
                    if ($cekabsen->num_rows() < 1) {
                        $this->M_absensi->inputabsen($siswa['nik'], $siswa['kode_kelas'], $siswa['kode_jurusan']);
                    ...
                    // edit asen
                    $this->M_absensi->editabsen($siswa['nik'], $siswa['kode_kelas'], $siswa['kode_jurusan']);
                    $message = 'Berhasil edit absen';
                } else if ($this->input->post('aksi') == 'hapus') {
                    $this->M_absensi->hapusabsen($siswa['nik'], $siswa['kode_kelas'], $siswa['kode_jurusan']);
    ...
    public function exportabsen()
    {
        ...
        if ($this->input->get('cariabsen') != FALSE) {
            ...
            if ($this->input->get('nik') != FALSE) {
                $nik = $this->input->get('nik');
                $dataguru = $this->M_absensi->CariSiswa($nik, $idkelas, $idjurusan);
                // deskripsi pencarian dengan nik
                $desc = 'nik ' . $nik . ' ( ' . $namabulan[$bulan] . ' ' . $tahun . ' )';
            } else {
                // pencarian tanpa nik
                $dataguru = $this->M_absensi->dataguruByKelas($idkelas, $idjurusan);
                // deskripsi pencarian tanpa nik
            ...
            $dataguru = $this->M_absensi->dataguruByKelas(100, 100);
    ...
    public function rekap()
    {
        ...
        $this->load->view('templates/header', $data);
        $this->load->view('absenguru/rekap');
        $this->load->view('templates/footer');
    }
