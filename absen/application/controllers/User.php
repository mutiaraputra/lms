...
class User extends CI_Controller
{
    ...
    public function user_guru()
    {
        ...
        verifikasiuser($datauser['role_id'], $url);
        $data = [
            'title' => WEBNAME . 'User Login guru',
            'webname' => WEBNAME,
            'userguru' => $this->M_user->getuserguru(),
            'titleedit' => 'Ubah Log guru',
            'user' =>  $this->M_user->getUserById($this->session->userdata('id'))[0]

        ];
        $this->load->view('templates/header', $data);
        $this->load->view('user/userguru');
        $this->load->view('templates/footer');
    }
    ...
    public function editlogguru()
    {
        $this->form_validation->set_rules('nik', 'menu', 'required|trim', [
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->M_user->editguru();
            $this->session->set_flashdata('flash', ['alert' => 'success', 'message' => 'Berhasil edit Log user siswa']);
        }
        redirect('User/user_guru');
    }
    public function deletelogsiswa($id)
    {
        ...
        $this->session->set_flashdata('flash', ['alert' => 'success', 'message' => 'Berhasil Hapus']);
        redirect('User/user_siswa');
    }
}
